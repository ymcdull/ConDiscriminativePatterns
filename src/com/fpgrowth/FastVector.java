...
/**
 * Implements a fast vector class without synchronized methods. Replaces
 * java.util.Vector. (Synchronized methods tend to be slow.)
 * 
...
public class FastVector implements Copyable, Serializable {
	...
	public class FastVectorEnumeration implements Enumeration {
		...
		// @ private invariant m_SpecialElement>=0 ==>
		// m_Counter!=m_SpecialElement;
	...
	public final void addElement(Object element) {
		...
		if (m_Size == m_Objects.length) {
			...
			// ccc+=m_CapacityMultiplier * (m_Objects.length
			// +m_CapacityIncrement);
	...
	public final void insertElementAt(Object element, int index) {
		...
		if (m_Size < m_Objects.length) {
			System.arraycopy(m_Objects, index, m_Objects, index + 1, m_Size
					- index);
			...
			System.arraycopy(m_Objects, index, newObjects, index + 1, m_Size
					- index);
			m_Objects = newObjects;
		}
		m_Size++;
	...
	public final void setCapacity(int capacity) {
		...
		System.arraycopy(m_Objects, 0, newObjects, 0,
				Math.min(capacity, m_Size));
		m_Objects = newObjects;
